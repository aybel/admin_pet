...
const handleIntervalChange = (intervalId, day, checked) => {
  ...
  // Emitir los intervalos seleccionados al componente padre
  emit('update:selectedIntervals', selectedIntervals.value)
}
  ...
  >
    ...
    <template #item.monday="{ item }">
      <div v-if="item.intervals && item.intervals.length > 0" class="d-flex flex-column gap-2">
        <div 
          ...
          :key="`monday-${interval.id}`"
          class="d-flex align-center gap-2"
        >
          <input 
            :id="`monday-${interval.id}`"
            type="hidden" 
            :value="interval.id"
          >
          <VCheckbox
            :model-value="isIntervalSelected(interval.id, 'monday')"
            :label="interval.time_range"
            hide-details
            density="compact"
            @update:model-value="(checked) => handleIntervalChange(interval.id, 'monday', checked)"
          />
        </div>
      </div>
    ...
    <template #item.tuesday="{ item }">
      <div v-if="item.intervals && item.intervals.length > 0" class="d-flex flex-column gap-2">
        <div 
          v-for="interval in item.intervals" 
          :key="`tuesday-${interval.id}`"
          class="d-flex align-center gap-2"
        >
          <input 
            :id="`tuesday-${interval.id}`"
            type="hidden" 
            :value="interval.id"
          >
          <VCheckbox
            :model-value="isIntervalSelected(interval.id, 'tuesday')"
            :label="interval.time_range"
            hide-details
            density="compact"
    ...
    <template #item.wednesday="{ item }">
      <div v-if="item.intervals && item.intervals.length > 0" class="d-flex flex-column gap-2">
        <div 
          v-for="interval in item.intervals" 
          :key="`wednesday-${interval.id}`"
          class="d-flex align-center gap-2"
        >
          <input 
            :id="`wednesday-${interval.id}`"
            type="hidden" 
            :value="interval.id"
          >
          <VCheckbox
            :model-value="isIntervalSelected(interval.id, 'wednesday')"
            :label="interval.time_range"
            hide-details
            density="compact"
    ...
    <template #item.thursday="{ item }">
      <div v-if="item.intervals && item.intervals.length > 0" class="d-flex flex-column gap-2">
        <div 
          v-for="interval in item.intervals" 
          :key="`thursday-${interval.id}`"
          class="d-flex align-center gap-2"
        >
          <input 
            :id="`thursday-${interval.id}`"
            type="hidden" 
            :value="interval.id"
          >
          <VCheckbox
            :model-value="isIntervalSelected(interval.id, 'thursday')"
            :label="interval.time_range"
            hide-details
            density="compact"
    ...
    <template #item.friday="{ item }">
      <div v-if="item.intervals && item.intervals.length > 0" class="d-flex flex-column gap-2">
        <div 
          v-for="interval in item.intervals" 
          :key="`friday-${interval.id}`"
          class="d-flex align-center gap-2"
        >
          <input 
            :id="`friday-${interval.id}`"
            type="hidden" 
            :value="interval.id"
          >
          <VCheckbox
            :model-value="isIntervalSelected(interval.id, 'friday')"
            :label="interval.time_range"
            hide-details
            density="compact"
            @update:model-value="(checked) => handleIntervalChange(interval.id, 'friday', checked)"
          />
        </div>
      </div>
      <span v-else class="text-caption text-disabled">Sin horarios</span>
    ...
    <template #item.saturday="{ item }">
      <div v-if="item.intervals && item.intervals.length > 0" class="d-flex flex-column gap-2">
        <div 
          v-for="interval in item.intervals" 
          :key="`saturday-${interval.id}`"
          class="d-flex align-center gap-2"
        >
          <input 
            :id="`saturday-${interval.id}`"
            type="hidden" 
            :value="interval.id"
          >
          <VCheckbox
            :model-value="isIntervalSelected(interval.id, 'saturday')"
            :label="interval.time_range"
            hide-details
            density="compact"
    ...
    <template #item.sunday="{ item }">
      <div v-if="item.intervals && item.intervals.length > 0" class="d-flex flex-column gap-2">
        <div 
          v-for="interval in item.intervals" 
          :key="`sunday-${interval.id}`"
          class="d-flex align-center gap-2"
        >
          <input 
            :id="`sunday-${interval.id}`"
            type="hidden" 
            :value="interval.id"
          >
          <VCheckbox
            :model-value="isIntervalSelected(interval.id, 'sunday')"
            :label="interval.time_range"
            hide-details
            density="compact"
            @update:model-value="(checked) => handleIntervalChange(interval.id, 'sunday', checked)"
          />
        </div>
      </div>
      <span v-else class="text-caption text-disabled">Sin horarios</span>
